...
<center>
 ...
 if(isset($_SESSION['user_login'])) //check employee login user not access in "employee_home.php" page
 {
                ...
                try{
                  ...
                  else if(!isset($errorMsg))
                  {
                    ...
                    */
                     ...
                     $u1r = TRUE;
                     $u2r = FALSE;
                     $insert_stmt=$db->prepare("INSERT INTO dm(title, user1, user2, message, timestamp, user1read, user2read) VALUES(:utitle,:uuser1,:uuser2,:umessage,:utimestamp,:uuser1read,:uuser2read)"); //sql insert query
                     $insert_stmt->bindParam(':utitle',$title);
                     $insert_stmt->bindParam(':uuser1',$_SESSION['user_login']);     //bind all parameter
                     $insert_stmt->bindParam(':uuser2',$row->email);
                     $insert_stmt->bindParam(':umessage',$message);
                     echo "--date=".$date;
                     $insert_stmt->bindParam(':utimestamp', $date);
                     $insert_stmt->bindParam(':uuser1read', intval($u1r));
                     $insert_stmt->bindParam(':uuser2read', intval($u2r));
                     echo "--title=".$title;
                     ...
                     if($insert_stmt->execute())
                     {
                      ...
                      <a href="employee_message.php">List of my Personal messages</a></div>
                      <?php
                     }
                  }
                }
...
?>
                <div class="foot"><a href="employee_message.php">Go to my messages</a></div>
        </body>
</html>
