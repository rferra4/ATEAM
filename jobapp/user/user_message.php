...
$sql1 = 'SELECT `title`, `user1`, `user2`, `message`, `timestamp`, `user2read`, `dmid` FROM `dm` WHERE user2 = :user AND user2read=0';
$req1 = $db->prepare($sql1);
$req1->bindParam(':user', $_SESSION['user_login']);
$req1->execute();
$sql2 = 'SELECT `title`, `user1`, `user2`, `message`, `timestamp`, `user2read`, `dmid` FROM `dm` WHERE user2 = :user AND user2read=1';
...
while($dn1 = $req1->fetch(PDO::FETCH_OBJ))
{
  ...
  while($dn3 = $req3->fetch(PDO::FETCH_OBJ))
  {
    ?>
            <tr>
            <td class="left"><a href="read_pm.php?dmid=<?php echo $dn1->dmid; ?>"><?php echo htmlentities($dn1->title, ENT_QUOTES, 'UTF-8'); ?></a></td>
            <td><a href="profile.php?id=<?php echo $dn3->id; ?>"><?php echo htmlentities($dn1->user1, ENT_QUOTES, 'UTF-8'); ?></a></td>
...
?>
        <tr>
        <td class="left"><a href="read_pm.php?dmid=<?php echo $dn2->dmid; ?>"><?php echo htmlentities($dn2->title, ENT_QUOTES, 'UTF-8'); ?></a></td>
        <td><a href="profile.php?id=<?php echo $dn3->id; ?>"><?php echo htmlentities($dn2->user1, ENT_QUOTES, 'UTF-8'); ?></a></td>
